...
This guide enables GPU (CUDA) acceleration with Linux (Kubuntu and other Ubuntu versions).  Kubuntu is a Linux version which has a Windows like environment and is what PixInsight recommends. It now supports the ONNX runtime libraries which simplifies the install making the process much shorter and easier. It is updated to support Kubuntu 26.04 LTS.  It also supports Kubuntu 24.04 LTS. The old guide which supports legacy Tensorflow instructions is at the end of these instructions under Archived Instructions including Tensorflow library install
