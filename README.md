...
I selected Kubuntu to install alongside Windows 11. I then dragged the divider to allocate the amount of drive space I wanted for Kubuntu. This will install a boot option (GRUB) menu when you boot your computer. During installation you are also given the option as to how to format your Kubuntu (root) partition. The default format is ext4. Ext4 is stable but there is a newer format called btrfs which would enable snapshots of your root partition allowing you to timeshift (roll back) to a previous state if an instability occurs. Note that this is not a true backup ..only a snapshot.  I formatted my main system drive as btrfs and I have a separate drive for my astronomy data formatted as ext4.
